...
from PyQt6.QtWidgets import (
    ...
    QLabel, QHBoxLayout, QSlider, QGridLayout,QHeaderView
)
...
import sys, os, keyboard, subprocess, tempfile
...
class VideoInterface(QWidget):
    ...
    def setup_ui(self):
        ...
        # 显示临时时间段
        self.temp_time_label = BodyLabel(f"时间段: {self.temp_start_time} - {self.temp_end_time}")
        ...
        # 生成ffmpeg命令按钮
        self.generate_ffmpeg_button = PushButton("生成FFmpeg命令", self.time_segment_card)
    ...
    def update_segments_display(self):
        ...
        if self.time_segments:
            # segments_text = "时间片段列表:\n"
            # for i, (start, end) in enumerate(self.time_segments, 1):
            #     segments_text += f"{i}. {start} - {end}\n"
            # self.segments_label.setText(segments_text.rstrip())
            print("ok")
        else:
    ...
    def generate_ffmpeg_command(self):
        ...
        output_path = os.path.join(os.path.dirname(self.video_path), f"{input_name}_剪辑_{timestamp}.mp4")
